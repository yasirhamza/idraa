...
A foreign key to ``users.id`` without ``ondelete`` is NO ACTION: deleting a
...
def test_every_no_action_user_fk_is_pre_checked() -> None:
    fks = _user_fks()
    no_action = {(t, c) for t, c, ondelete in fks if ondelete is None}
    assert no_action, "introspection found no NO-ACTION user FKs; guard would be vacuous"
    covered = {(model.__table__.name, col.key) for model, col in _HISTORY_COLUMNS}
