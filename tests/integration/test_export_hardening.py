"""Export hardening parity (idraa#107 + idraa#110).

Four hardening gaps closed together:

1. #107(1) — the two pre-category export routes (`/scenarios/{id}/export`,
   `/runs/{id}/control-matrix.csv`) gain the EXPORTS step-up gate their
   sibling exports already carry.
2. #107(2) — /healthz exposes the security-settings cache state
   ("warm"/"cold") so an operator can see when a boot-warm failure has left
   env-fallback policy in effect. Deliberately DB-free: healthz must stay
   cheap during the boot-write window (#72) and pre-setup "cold" is normal.
3. #110(1) — bulk-export audit rows record the TRUSTED client IP (edge
   header machinery from routes/deps.py) instead of the direct socket peer,
   falling back to the peer when no trust strategy is configured.
4. #110(2) — the shared csv_response helper (and the scenario JSON export)
   emit ``Cache-Control: private, no-store``, matching the PDF-report and
   samples-export precedents.
"""
...
@pytest.mark.asyncio
async def test_exports_off_leaves_scenario_single_export_open(
    authed_admin: tuple[AsyncClient, uuid.UUID], db_session: AsyncSession
) -> None:
    """Category OFF: the route must not gate (404 for the dummy id, not 303)."""
    client, org_id = authed_admin
    await _apply(db_session, org_id, step_up_exports=False)
    await _make_stale(db_session, client)
    r = await client.get(f"/scenarios/{uuid.uuid4()}/export", follow_redirects=False)
...
async def _export_audit_ip(
    ...
    assert rows, "no scenario.export audit row written"
    return rows[-1].ip_address
...
async def test_bulk_export_audit_ignores_unconfigured_header(
    ...
    assert ip != "198.51.100.99"  # falls back to the direct peer (or None)
