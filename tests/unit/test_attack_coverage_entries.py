...
native lognormal mean = mu_s + ln(Sum shares), sigma = WITHIN_SCENARIO_SIGMA_DEFAULT (post sigma-recalibration; was sigma_s) for W1, the
...
# `_T0_MEDIAN_OVERRIDES` -- keep all three textually identical.
...
def test_new_entry_reconstructs_from_envelope_and_shares(slug: str) -> None:
    ...
    # mu-legs stay envelope x shares EXCEPT the Task-0 wiper override (see
    # _T0_MEDIAN_OVERRIDES above).
