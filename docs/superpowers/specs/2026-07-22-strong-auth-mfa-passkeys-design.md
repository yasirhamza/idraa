...
> **Config-driven RP-ID + a WebAuthn protocol note (plan-gate 2026-07-22).** `WEBAUTHN_RP_ID` / `WEBAUTHN_ORIGINS` are per-deployment configuration (Settings/env), never fixed in code — each deployment sets its own; the shipped default (`idraa.fly.dev` + one matching origin) is just one coherent example that a self-hoster overrides. Protocol constraint to keep in view (a WebAuthn rule, NOT a code limitation): a credential binds to ONE RP-ID, which must be a registrable-domain suffix of the origin, so a single deployment cannot serve passkeys on two *different* registrable domains at once (e.g. `idraa.fly.dev` AND `idraa.app`). Normal deployments front the product on one canonical origin (config handles this fully); serving passkeys on a second registrable domain from the same deploy would need WebAuthn **Related Origin Requests** (a `/.well-known/webauthn` file), a post-P1 option. The default MUST be internally consistent (every origin covered by the RP-ID) or the prod validator rejects it — which is the sole reason the default is single-origin rather than the earlier two-origin value.
