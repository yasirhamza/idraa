...
**Files:**
- Modify: `src/idraa/services/chart_svg.py` — each series dict gained by `dual_curve`/`epc_curve` (and thus the single wrappers) adds `area_d`: the series `path_d` extended `L <last_x>,<baseline> L <first_x>,<baseline> Z` where baseline = the shared plot-bottom constant `round(VIEW_H - MARGIN["bottom"], 1)` (computed per-function — EPC never calls `_y_scale`). Pure geometry, None-safe (empty series → no area_d key or None).
