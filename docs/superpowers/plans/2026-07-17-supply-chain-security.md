...
**Files:**
- Procedure: enable CodeQL default setup (API call — no workflow file; owner
  decision 2026-07-17, see design drift log)
...
- [ ] **Step 1: Enable CodeQL default setup (advisory)**

```bash
gh api -X PATCH repos/yasirhamza/idraa/code-scanning/default-setup \
  -f state=configured -f query_suite=default
gh api repos/yasirhamza/idraa/code-scanning/default-setup   # verify state=configured, python detected
```

GitHub manages the analysis workflow, triggers, and CodeQL versions (anti-fossil
by construction). ADVISORY — do NOT add a `code_scanning` ruleset (AndroDR
lesson stands). If the API rejects `query_suite=default`, retry without the
field (repo language auto-detection handles python).
...
```bash
for f in .github/workflows/dependency-review.yml .github/dependency-review-config.yml; do
  uv run python -c "import yaml; yaml.safe_load(open('$f')); print('$f OK')"
done
git add .github/workflows/dependency-review.yml .github/dependency-review-config.yml
git commit -m "ci: delta dependency-review gate + allow-ghsas convention; CodeQL default setup enabled via API (#555)"
```
