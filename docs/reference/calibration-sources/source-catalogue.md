...
| `netdiligence_2025` | NetDiligence Cyber Claims Study 2025 | loss-magnitude (reference band only) | paginated | Table 3 (printed p.52, incident cost by revenue size), Table 9 (printed p.59, cause of loss — SMEs), p.7 (population + incident-cost definition + payout ratios), p.50 (14 contributing insurers) | **NOT a loss anchor of ANY tier** despite page-citability — claims-process-visible incident cost ≠ FAIR loss magnitude (conditioning register in the source note); σ-consistency signals + reference bands ONLY, direction not established, NEVER percentile anchors (PR3 D24). PDF licensed + untracked (gitignored, `*.pdf` deny-glob). |
